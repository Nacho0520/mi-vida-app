...
export default function SettingsModal({ isOpen, onClose, user, appVersion, isPro, onUpgrade }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      const { data } = await supabase
        ...
        .eq('id', user.id)
        .maybeSingle()
      setResolvedPlan(data?.plan === 'pro' ? 'pro' : 'free')
      ...
      <div className="w-full max-w-sm bg-neutral-800/80 radius-card p-6 shadow-apple border border-white/5 relative backdrop-blur-xl">
        ...
        <div className="premium-divider">
          ...
          <div className="bg-neutral-800/60 rounded-2xl border border-white/5 p-4 mb-4">
            <p className="text-xs font-semibold text-neutral-400 mb-3">{t('subscription_title')}</p>
            ...
            {resolvedPlan === null ? (
              ...
              <>
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <p className="text-white font-bold text-sm">{t('plan_pro_title')}</p>
                </div>
                <p className="text-neutral-400 text-xs mb-3">{t('plan_pro_desc')}</p>
                <div className="space-y-1.5">
                  {[
                    ...
                    <div key={i} className="flex items-center gap-1.5">
                      ...
                      <span className="text-[11px] text-neutral-300">{benefit}</span>
                    </div>
                  ))}
                </div>
              </>
              ...
              /* ── Estado FREE ── */
              <>
                <p className="text-neutral-400 font-medium text-sm mb-1">{t('plan_free_title')}</p>
                <p className="text-neutral-500 text-xs mb-4">{t('plan_free_desc')}</p>
